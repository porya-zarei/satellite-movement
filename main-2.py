...
def check_points_are_same(point1, point2, scale):
    return int(point1[1] * (scale / 1000)) == int(point2[1] * (scale / 1000))
...
def tilt_point(point, tilt_angle, angle_step, distance):
    point = get_shifted_point(point, distance, tilt_angle, angle_step, angle_step > 0)
    return point
...
def fill_earth(
    ...
    for i in range(df.shape[0]): 
        ...
        point = tilt_point(org_point, angle_step, selected_angle_step, limit)
        prev_point = point
        if i>0:
            prev_point = [df.iloc[i-1]["Longitude"], df.iloc[i-1]["Latitude"]]
        angle = 55 if org_point[1] > prev_point[1] else 135
        # print("step => ",selected_angle_step)
        points = get_points(limit, point, angle)
        if check_points_are_same(point, check_point, scale):
            ...
            # print("selected_angle_step => ", selected_angle_step)
            point = tilt_point(
                point,
                angle_step,
                selected_angle_step,
...
def update_map(
    ...
    region_df = filter_region(df.copy(), region_min, region_max, 1)
...
def main():
    ...
    map_image_name = "map_mat_image_2"
